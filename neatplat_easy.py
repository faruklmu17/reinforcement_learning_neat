...
colwin.topleft = (round(SCwidth * 0.50), floor_y - win.get_height())  # EASY: Goal at 50%
...
acsellaration = 1.01
enemyX = randint(int(SCwidth*0.35), int(SCwidth*0.45))  # EASY: Random spawn 35-45%
...
def enemy_move():
    ...
    if X > enemyX:
        enemyX += SCwidth*((0.1603 + ((random()*0.01)-0.005))/1536) * 0.5  # EASY: 50% speed
        enemyISR = 1
        enemyISL = 0
    if X < enemyX:
        enemyX -= SCwidth*((0.1603 + ((random()*0.01)-0.005))/1536) * 0.5  # EASY: 50% speed
...
#     If seed is provided, the enemy spawn becomes deterministic so training/testing
#     is consistent across runs.
#     """
#     global X, Y, cooldown, gravity, acsellaration, enemyX, enemyY, enemyISR, enemyISL, fly, prev_Y, prev_enemyX

#     # Use a deterministic RNG when seed is provided
...
#     # place enemy on the floor (top = floor_y - enemy height)
#     enemyX = rng.randint(int(SCwidth * 0.35), int(SCwidth * 0.55))
#     enemyY = floor_y - enemy1.get_height()
