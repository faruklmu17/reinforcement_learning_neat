...
colwin.topleft = (round(SCwidth * 0.60), floor_y - win.get_height())  # MEDIUM: Goal at 60%
...
acsellaration = 1.01
enemyX = randint(int(SCwidth*0.35), int(SCwidth*0.50))  # MEDIUM: Random spawn 35-50%
...
def enemy_move():
    ...
    if X > enemyX:
        enemyX += SCwidth*((0.1603 + ((random()*0.01)-0.005))/1536) * 0.75  # MEDIUM: 75% speed
        enemyISR = 1
        enemyISL = 0
    if X < enemyX:
        enemyX -= SCwidth*((0.1603 + ((random()*0.01)-0.005))/1536) * 0.75  # MEDIUM: 75% speed
...
def sim(A,D,W):
    ...
    if W and cooldown <= 0 or not fly == 0:
        ...
        if fly == 300:  # MEDIUM: 300 frame jump
            fly = 0
